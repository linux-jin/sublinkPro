...
| 🧦 **SOCKS5 网关** | 健康感知 TCP CONNECT 网关，支持候选节点池、故障切换、连接限制和管理员实时监控 | [📖](docs/features/socks5.zh-CN.md) |
...
- SOCKS5 新增按分组、来源、协议和国家/地区过滤的候选节点池。
- 指定 SOCKS5 出站节点支持关键词搜索、分组筛选和服务端分页加载。
- 主动健康结果接入选路，`best` 优先使用探测延迟，并在存在替代节点时跳过探测失败节点。
