...
| 🧦 **SOCKS5 网关** | 健康感知 TCP CONNECT 网关，支持候选节点池、粘性会话、P2C 智能选路、故障切换、连接限制和实时监控 | [📖](docs/features/socks5.zh-CN.md) |
...
- SOCKS5 新增 P2C 智能选路，综合主动探测延迟、活动连接负载和连续失败惩罚。
- 新增有界的逐节点运行时计数，并精确跟踪活动连接生命周期。
- 智能选路保留粘性会话优先级，并按评分排列后续回退节点。
