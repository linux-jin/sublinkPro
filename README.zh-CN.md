...
- 新增独立智能分组，可跨原分组按已保存国家**或**节点名称中的国家线索筛选，支持原分组限制。
- 支持候选节点检测、动态延迟/速度及有效期条件、逐节点排除原因，并可在弹框中批量或单节点测速。
- 智能分组可直接用于动态/混合订阅，不移动原节点；订阅界面可清楚查看并选择关联的智能分组。

v1.13.0 新增独立的国家/名称智能分组及订阅集成。匹配规则和测速要求详见[智能分组指南](docs/features/smart-groups.zh-CN.md)。
