...
| 🧦 **SOCKS5 网关** | 健康感知 TCP CONNECT 网关，支持候选节点池、粘性会话、故障切换、连接限制和实时监控 | [📖](docs/features/socks5.zh-CN.md) |
...
- SOCKS5 新增按客户端 IP 或已认证用户名绑定的内存粘性会话。
- 新增 60-604800 秒可配置滑动 TTL，每次连接成功后自动刷新。
- 粘性节点异常、进入冷却、链接变化或离开候选池时会自动重新选路。
