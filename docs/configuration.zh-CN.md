...
管理员可通过侧边栏的 **系统设置 -> SOCKS5 网关** 启用本地 TCP SOCKS5 CONNECT 监听。网关默认关闭并绑定 `127.0.0.1:1080`，密码使用 API 加密密钥加密保存。配置保存在数据库中，不使用环境变量或 `config.yaml`。一期支持最佳、随机或指定节点转发，不支持 UDP。
