...
管理员可通过侧边栏的 **系统设置 -> SOCKS5 网关** 启用本地 TCP SOCKS5 CONNECT 监听。网关默认关闭并绑定 `127.0.0.1:1080`，密码使用 API 加密密钥加密保存。配置保存在数据库中，不使用环境变量或 `config.yaml`。网关支持最佳、随机、轮询或指定节点转发，可按分组、来源、协议和国家/地区过滤候选节点池，并支持配置重试次数、拨号超时、失败节点冷却、指定节点失败回退、连接数限制、空闲与最大时长限制、实时连接监控和节点主动健康探测。主动探测结果会参与选路：探测失败的节点会在存在替代节点时被跳过，最佳节点策略会优先使用探测延迟排序。非回环监听必须启用认证。目前不支持 UDP。
