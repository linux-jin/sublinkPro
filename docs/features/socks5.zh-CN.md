...
现有网关设置会自动成为不可删除的 `default` 路由配置，升级时不需要迁移数据。其他 Profile 通过认证用户名选择：原始用户名继续使用 `default`；`username@japan` 选择 `japan` Profile 并按客户端 IP 粘性；`username@japan.user01` 选择同一 Profile 并按 `user01` 账号粘性。所有形式仍使用现有网关密码。禁用或不存在的 Profile 会导致认证失败；使用多 Profile 选路必须开启用户名/密码认证。轮询计数和粘性租约按 Profile 隔离，而节点活动负载和健康状态在 Profile 之间共享，避免同一节点被重复压满。
...
- `GET /api/v1/settings/socks5/connections` — 返回当前活动连接数组，每项包含 `id`、`clientAddress`、`target`、`nodeName`、`profileId`、`profileName`、可选 `account`、`phase`、`startedAt`、`lastActivity`、`uploadBytes` 和 `downloadBytes`。
